...
contract("LogFactory basic test patterns", function(accounts) {

  owner = accounts[0];

  it('create LogFactory, Pilot, Logbook and Document contracts', async function() {

    logFactoryContract = await LogFactory.new()
		await logFactoryContract.createPilotContract(accounts[0], {from: owner})
		pilotContractAddress = await logFactoryContract.getPilotContract(accounts[0], {from: owner})
    pilotContract = await Pilot.at(pilotContractAddress)
    ...
    assert.strictEqual(logFactoryContract.address, pilotContractOwner, "Pilot contract addresses do not match!");
  ...
  it("create pilot data and get it back.", async function() {
    ...
    const pilotData = await logFactoryContract.getPilotData({from: owner});
  ...
  it("create a logbook entry and get it back.", async function() {
    ...
    const documentEntry = await logFactoryContract.getIpfsLogbook(
      1,
      {from: owner}
    )
  ...
  it("delete a logbook entry and get it back.", async function() {
    await logFactoryContract.deleteIpfsLogbook(
      1,
      {from: owner}
    )
    const documentEntry = await logFactoryContract.getIpfsLogbook(
  ...
  it("delete a document entry and get it back.", async function() {
    await logFactoryContract.deleteIpfsDocument(
      1,
      {from: owner}
    )
    const documentEntry = await logFactoryContract.getIpfsDocument(
